'use strict';

module.exports = {
  up: (queryInterface, Sequelize) => {
    return queryInterface.bulkInsert('pais', [
      {
        nombre: 'Afganistán',
		nacionalidad: 'Afgano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Albania',
		nacionalidad: 'Albanés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Alemania',
		nacionalidad: 'Alemán',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Andorra',
		nacionalidad: 'Andorrano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Angola',
		nacionalidad: 'Angoleño',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Anguila',
		nacionalidad: 'Anguilense',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Antigua y Barbuda',
		nacionalidad: 'Antiguano y Barbudano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Aotearoa Nueva Zelanda',
		nacionalidad: 'Neozelandés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Arabia Saudí',
		nacionalidad: 'Saudí o Saudita',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Argelia',
		nacionalidad: 'Argelino',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Argentina',
		nacionalidad: 'Argentino',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Armenia',
		nacionalidad: 'Armenio',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Aruba',
		nacionalidad: 'Arubeño',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Australia',
		nacionalidad: 'Australiano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Austria',
		nacionalidad: 'Austriaco',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Azerbaiyán',
		nacionalidad: 'Azerbaiyano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Bahamas',
		nacionalidad: 'Bahameño',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Bahréin',
		nacionalidad: 'Bareiní',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Bangladesh',
		nacionalidad: 'Bengalí',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Barbados',
		nacionalidad: 'Barbadeño',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Bélgica',
		nacionalidad: 'Belga',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Belice',
		nacionalidad: 'Beliceño',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Benín',
		nacionalidad: 'Beninés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Bermudas',
		nacionalidad: 'Bermudeño',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Bielorrusia',
		nacionalidad: 'Bielorruso',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Bolivia',
		nacionalidad: 'Boliviano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Bosnia y Herzegovina',
		nacionalidad: 'Bosnio o Herzegovino',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Botsuana',
		nacionalidad: 'Botsuano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Brasil',
		nacionalidad: 'Brasileño',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Brunéi Darussalam',
		nacionalidad: 'Bruneano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Bulgaria',
		nacionalidad: 'Búlgaro',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Burkina Faso',
		nacionalidad: 'Burkinés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Burundi',
		nacionalidad: 'Burundés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Bután',
		nacionalidad: 'Butanés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Cabo Verde',
		nacionalidad: 'Caboverdiano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Camboya',
		nacionalidad: 'Camboyano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Camerún',
		nacionalidad: 'Camerunés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Canadá',
		nacionalidad: 'Canadiense',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Chad',
		nacionalidad: 'Chadense',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Chile',
		nacionalidad: 'Chileno',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Chinese Taipei',
		nacionalidad: 'Taiwanés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Chipre',
		nacionalidad: 'Chipriota',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Colombia',
		nacionalidad: 'Colombiano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Comoras',
		nacionalidad: 'Comorense',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Congo',
		nacionalidad: 'Congoleño',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Costa de Marfil',
		nacionalidad: 'Marfileño',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Costa Rica',
		nacionalidad: 'Costarricense',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Croacia',
		nacionalidad: 'Croata',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Cuba',
		nacionalidad: 'Cubano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Curazao',
		nacionalidad: 'Curazoleño',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Dinamarca',
		nacionalidad: 'Danés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Dominica',
		nacionalidad: 'Dominicano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Ecuador',
		nacionalidad: 'Ecuatoriano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'EEUU',
		nacionalidad: 'Estadounidense',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Egipto',
		nacionalidad: 'Egipcio',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'El Salvador',
		nacionalidad: 'Salvadoreño',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Emiratos Árabes Unidos',
		nacionalidad: 'Emiratí',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Eritrea',
		nacionalidad: 'Eritreo',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Escocia',
		nacionalidad: 'Escocés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Eslovaquia',
		nacionalidad: 'Eslovaco',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Eslovenia',
		nacionalidad: 'Esloveno',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'España',
		nacionalidad: 'Español',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Estonia',
		nacionalidad: 'Estonio',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Esuatini',
		nacionalidad: 'Suazi',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Etiopía',
		nacionalidad: 'Etíope',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Filipinas',
		nacionalidad: 'Filipino',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Finlandia',
		nacionalidad: 'Finlandés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Fiyi',
		nacionalidad: 'Fiyiano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Francia',
		nacionalidad: 'Francés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Gabón',
		nacionalidad: 'Gabonés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Gales',
		nacionalidad: 'Galés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Gambia',
		nacionalidad: 'Gambiano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Georgia',
		nacionalidad: 'Georgiano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Ghana',
		nacionalidad: 'Ghanés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Gibraltar',
		nacionalidad: 'Gibraltareño',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Granada',
		nacionalidad: 'Granadino',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Grecia',
		nacionalidad: 'Griego',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Guam',
		nacionalidad: 'Guameño',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Guatemala',
		nacionalidad: 'Guatemalteco',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Guinea',
		nacionalidad: 'Guineano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Guinea Ecuatorial',
		nacionalidad: 'Ecuatoguineano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Guinea-Bissáu',
		nacionalidad: 'Bissauguineano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Guyana',
		nacionalidad: 'Guyanés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Haití',
		nacionalidad: 'Haitiano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Honduras',
		nacionalidad: 'Hondureño',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Hong Kong, China',
		nacionalidad: 'Hongkonés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Hungría',
		nacionalidad: 'Húngaro',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'India',
		nacionalidad: 'Indio',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Indonesia',
		nacionalidad: 'Indonesio',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Inglaterra',
		nacionalidad: 'Inglés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Irak',
		nacionalidad: 'Iraquí',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Irlanda del Norte',
		nacionalidad: 'Norirlandés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Islandia',
		nacionalidad: 'Islandés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Islas Caimán',
		nacionalidad: 'Caimanés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Islas Cook',
		nacionalidad: 'Cookiano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Islas Feroe',
		nacionalidad: 'Feroés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Islas Salomón',
		nacionalidad: 'Salomonense',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Islas Vírgenes Británicas',
		nacionalidad: 'Británico',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Islas Vírgenes Estadounidenses',
		nacionalidad: 'Estadounidense',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Israel',
		nacionalidad: 'Israelí',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Italia',
		nacionalidad: 'Italiano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Jamaica',
		nacionalidad: 'Jamaicano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Japón',
		nacionalidad: 'Japonés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Jordania',
		nacionalidad: 'Jordano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Kazajstán',
		nacionalidad: 'Kazajo',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Kenia',
		nacionalidad: 'Keniano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Kosovo',
		nacionalidad: 'Kosovar',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Kuwait',
		nacionalidad: 'Kuwaití',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Laos',
		nacionalidad: 'Laosiano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Lesoto',
		nacionalidad: 'Lesotense',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Letonia',
		nacionalidad: 'Letón',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Líbano',
		nacionalidad: 'Libanés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Liberia',
		nacionalidad: 'Liberiano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Libia',
		nacionalidad: 'Libio',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Liechtenstein',
		nacionalidad: 'Liechtensteiniano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Lituania',
		nacionalidad: 'Lituano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Luxemburgo',
		nacionalidad: 'Luxemburgués',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Macao',
		nacionalidad: 'Macaense',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Macedonia del Norte',
		nacionalidad: 'Macedonio del Norte',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Madagascar',
		nacionalidad: 'Malgache',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Malasia',
		nacionalidad: 'Malasio',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Malaui',
		nacionalidad: 'Malauí',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Maldivas',
		nacionalidad: 'Maldivo',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Mali',
		nacionalidad: 'Maliense',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Malta',
		nacionalidad: 'Maltés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Marruecos',
		nacionalidad: 'Marroquí',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Mauricio',
		nacionalidad: 'Mauriciano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Mauritania',
		nacionalidad: 'Mauritano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'México',
		nacionalidad: 'Mexicano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Moldavia',
		nacionalidad: 'Moldavo',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Mongolia',
		nacionalidad: 'Mongol',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Montenegro',
		nacionalidad: 'Montenegrino',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Montserrat',
		nacionalidad: 'Montserratiano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Mozambique',
		nacionalidad: 'Mozambiqueño',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Myanmar',
		nacionalidad: 'Birmano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Namibia',
		nacionalidad: 'Namibio',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Nepal',
		nacionalidad: 'Nepalí',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Nicaragua',
		nacionalidad: 'Nicaragüense',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Níger',
		nacionalidad: 'Níger',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Nigeria',
		nacionalidad: 'Nigeriano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Noruega',
		nacionalidad: 'Noruego',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Nueva Caledonia',
		nacionalidad: 'Neocaledonio',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Omán',
		nacionalidad: 'Omaní',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Países Bajos',
		nacionalidad: 'Neerlandés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Pakistán',
		nacionalidad: 'Pakistaní',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Palestina',
		nacionalidad: 'Palestino',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Panamá',
		nacionalidad: 'Panameño',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Papúa Nueva Guinea',
		nacionalidad: 'Papua Neoguineano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Paraguay',
		nacionalidad: 'Paraguayo',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Perú',
		nacionalidad: 'Peruano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Polonia',
		nacionalidad: 'Polaco',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Portugal',
		nacionalidad: 'Portugués',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Puerto Rico',
		nacionalidad: 'Puertorriqueño',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Qatar',
		nacionalidad: 'Catarí',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'RD del Congo',
		nacionalidad: 'Congoleño',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'RDP de Corea',
		nacionalidad: 'Norcoreano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'República Centroafricana',
		nacionalidad: 'Centroafricano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'República Checa',
		nacionalidad: 'Checo',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'República de Corea',
		nacionalidad: 'Surcoreano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'República de Irlanda',
		nacionalidad: 'Irlandés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'República Dominicana',
		nacionalidad: 'Dominicano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'República Kirguisa',
		nacionalidad: 'Kirguís',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'RI de Irán',
		nacionalidad: 'Iraní',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'RP China',
		nacionalidad: 'Chino',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Ruanda',
		nacionalidad: 'Ruandés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Rumanía',
		nacionalidad: 'Rumano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Rusia',
		nacionalidad: 'Ruso',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Samoa',
		nacionalidad: 'Samoano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Samoa Estadounidense',
		nacionalidad: 'Samoano Estadounidense',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'San Cristóbal y Nieves',
		nacionalidad: 'San Cristobalense y Nevisiano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'San Marino',
		nacionalidad: 'Sanmarinense',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'San Vicente y las Granadinas',
		nacionalidad: 'Sanvicentino y Granadino',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Santa Lucía',
		nacionalidad: 'Santa Lucense',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Santo Tomé y Príncipe',
		nacionalidad: 'Santotomense',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Senegal',
		nacionalidad: 'Senegalés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Serbia',
		nacionalidad: 'Serbio',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Seychelles',
		nacionalidad: 'Seychellense',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Sierra Leona',
		nacionalidad: 'Sierra Leones',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Singapur',
		nacionalidad: 'Singapurense',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Siria',
		nacionalidad: 'Sirio',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Somalia',
		nacionalidad: 'Somalí',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Sri Lanka',
		nacionalidad: 'Sri Lanka (ceilanés)',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Sudáfrica',
		nacionalidad: 'Sudafricano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Sudán',
		nacionalidad: 'Sudanés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Sudán del Sur',
		nacionalidad: 'Sursudanés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Suecia',
		nacionalidad: 'Sueco',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Suiza',
		nacionalidad: 'Suizo',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Surinam',
		nacionalidad: 'Surinamés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Tahití',
		nacionalidad: 'Tahitiano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Tailandia',
		nacionalidad: 'Tailandés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Tanzania',
		nacionalidad: 'Tanzano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Tayikistán',
		nacionalidad: 'Tayiko',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Timor Oriental',
		nacionalidad: 'Timorense',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Togo',
		nacionalidad: 'Togolés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Tonga',
		nacionalidad: 'Tongano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Trinidad y Tobago',
		nacionalidad: 'Trinidadense y Tobagoniano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Túnez',
		nacionalidad: 'Tunecino',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Turcas y Caicos',
		nacionalidad: 'Turcocaimán',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Turkmenistán',
		nacionalidad: 'Turcomano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Turquía',
		nacionalidad: 'Turco',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Ucrania',
		nacionalidad: 'Ucraniano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Uganda',
		nacionalidad: 'Ugandés',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Uruguay',
		nacionalidad: 'Uruguayo',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Uzbekistán',
		nacionalidad: 'Uzbeko',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Vanuatu',
		nacionalidad: 'Vanuatuense',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Venezuela',
		nacionalidad: 'Venezolano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Vietnam',
		nacionalidad: 'Vietnamita',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Yemen',
		nacionalidad: 'Yemení',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Yibuti',
		nacionalidad: 'Yibutiano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Zambia',
		nacionalidad: 'Zambiano',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
        nombre: 'Zimbabue',
		nacionalidad: 'Zimbabuense',
        created_at: new Date(),
        updated_at: new Date()
      }
    ]);
  },

  down: (queryInterface, Sequelize) => {
    return queryInterface.bulkDelete('pais', null, {});
  },

  order: 2,
};
